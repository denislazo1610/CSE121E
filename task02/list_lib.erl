-module(list_lib).
-export([shuffle/1, start/0]).

start()->
   % Nombre = "lNOpe",

   Names = ["Ayah", "Herman", "Conor", "Randall", "Naseem", "Simmonds", "Kurtis", "Holcomb", "Ozan", "Arnold", "Saira", "Gibbons", "Malaika", "Bush", "Jordanne", "Neal", "Roisin", "Sinclair", "Kirby", "Hough"],
   Gender = ["male", "female"],

   % Valores = list_lib:shuffle(Names),
   
   Valores = [{X, Y, Z} || Y <- list_lib:shuffle(Names), X <- [1] , Z <- list_lib:shuffle(Gender)],
   io:fwrite("~p~n",[Valores]).
